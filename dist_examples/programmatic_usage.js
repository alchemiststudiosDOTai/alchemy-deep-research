...
import { deepResearch, generateReport, sanitizeQueryForFilename // Import the utility
...
async function main() {
    ...
    try {
        const researchResult = await deepResearch(topic, breadth, depth, concurrency, options);
        console.log("\n--- Research Learnings ---");
        ...
        const summaryReportMarkdown = await generateReport(topic, researchResult.learnings, researchResult.visited, options.openaiModel // Use the same model for consistency
        );
